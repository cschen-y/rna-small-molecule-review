...
from data_process.msa import get_msa
...
def get_pdb_features(pdb_file_path='./data/data_apo_holo_pdb_adjust',fasta_file_path="./data/apo_holo_fasta",msa_file_path='./data/apo_holo_msa/aglin',em_file_path='./data/apo_holo_feature/apo_holo_em', asa_file_path = "./data/apo_holo_feature/apo_holo_asa",top_k=8):
    ...
    all_feature_s,edge_s = get_edge_and_feature(fasta_file_path,"./re_test/secondary_structure")
    all_msa = []
    all_topology = []
    all_rna_type = []
    all_feature = []
    ...
    for pdb in fasta_list:
        ...
        msa = get_msa(f'{msa_file_path}/{pdb[:5]}_msa_aligned.aln')
        print(msa)
...
class TestDataset(InMemoryDataset):
    def __init__(self, root, pdb_file_path, fasta_file_path,msa_file_path,em_file_path,asa_file_path,label_file_path,top_k, mode="train",transform=None, pre_transform=None, pre_filter=None):
    ...
    def process(self):
        data_list = []
        all_features, all_edges, all_labels, all_edge_s=  get_pdb_features(self.pdb_file_path,self.fasta_file_path,self.msa_file_path,self.em_file_path,self.asa_file_path,self.top_k)
